...
var aicontextCmd = &cobra.Command{
	...
	Long: "Generate structured documentation about iconforge for use by AI tools.\n" +
		"\n" +
		"Outputs a markdown (or JSON) reference of all commands, flags, and usage\n" +
		"patterns that AI assistants can consume as context.\n" +
		"\n" +
		"Examples:\n" +
		"  iconforge aicontext             # Markdown output (default)\n" +
		"  iconforge aicontext --json      # Structured JSON\n" +
		"  iconforge aicontext --compact   # Shorter output",
	RunE: runAIContext,
}
...
func aiWriteCompactCommand(b *strings.Builder, cmd *cobra.Command, prefix string) {
	...
	_, _ = fmt.Fprintf(b, "- `iconforge %s` - %s", name, cmd.Short)
